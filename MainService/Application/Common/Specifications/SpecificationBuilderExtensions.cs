...
public static class SpecificationBuilderExtensions
{
    public static ISpecificationBuilder<T> SearchBy<T>(this ISpecificationBuilder<T> query, BaseFilter filter) =>
        query
            .SearchByKeyword(filter.Keyword, filter.AdvancedSearch)
    ...
    {
        if (advancedSearch == null || advancedSearch.Fields.Count == 0)
        {
            return specificationBuilder.AdvancedSearch(new Search { Keyword = keyword });
        }
        else
        {
            return specificationBuilder.AdvancedSearch(new Search { Keyword = keyword, Fields = advancedSearch.Fields });
        }
    }
